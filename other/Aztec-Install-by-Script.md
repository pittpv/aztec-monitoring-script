...
Меню скрипта разбито на **категории**. В интерактивном терминале: `↑` / `↓` — навигация, `Enter` — выбор; либо введите номер опции `0–27` и нажмите `Enter` для быстрого перехода.

Для начала установки ноды:

1.  **Выберите категорию «Управление нодой» → опцию `13`**, либо сразу введите **`13`** и нажмите Enter.
...
> **Примечание:** Скриншот может показывать прежний плоский список опций; в актуальной версии меню категориальное, номера опций соответствуют разделу «Использование» в README.
...
Если выбран вариант с BLS ключами, используйте формат **три поля** (приватный SECP256K1, eth-адрес, приватный BLS):

`приватный_ключ_с_0x,адрес_валидатора,приватный_BLS_ключ_с_0x`

Пример:
`0xa1b2c3d4e5f6...,0x742d35Cc6634C0532925a3b844Bc454e4438f44e,0xa1b2c3d4e5f6...`

BLS хранится в **`keystore.json`**; отдельные файлы `bls_validator_*.yml` **не создаются**. Для web3signer по-прежнему создаются только `validator_N.yml` с ключом SECP256K1. Четвёртое значение в строке (публичный BLS), если вы его введёте, **игнорируется** — оставлено для обратной совместимости со старым форматом ввода.
...
> **Важно:** Скрипт автоматически создаст необходимые конфигурационные файлы: в `$HOME/aztec/keys` — `validator_N.yml` (SECP256K1 для web3signer), в `$HOME/aztec/config` — `keystore.json` (в т.ч. поля BLS при их наличии).

## Конфигурация Docker Compose

Для финальной настройки ноды скрипт запросит у вас данные для генерации файла `docker-compose.yml` (образ **Aztec v5**, например `5.0.0`):
...
*   **`ETHEREUM_DEBUG_HOSTS`** — debug RPC (по умолчанию тот же, что `ETHEREUM_RPC_URL`; можно оставить пустым и нажать Enter).
*   **`P2P_IP`** — этот параметр скрипт определит автоматически. **Если вы используете VPN на сервере**, то после завершения установки ноды необходимо ввести реальный ip-адрес **вручную** в файле `.env` в папке `$HOME/aztec`

> **Aztec v5:** compose создаётся **без** флага `--archiver`; тома: данные → `/data`, keystore → `/config`; `LOG_LEVEL: info;debug:node:sentinel`. На mainnet в `.env` также задаётся `GOVERNANCE_PROPOSER_PAYLOAD_ADDRESS` для signaling.
...
> *   Данные ноды будут храниться в разных директориях: `$HOME/.aztec/mainnet/data/` для mainnet и `$HOME/.aztec/testnet/data/` для testnet
...
1.  **Проверьте статус синхронизации** ноды, выбрав опцию `1` в меню скрипта.
2.  **Установите агент мониторинга** для удобного отслеживания работы ноды через опцию `9`.
3.  **Скачайте файл определения критических ошибок** с помощью опции `27`.
4.  **Проверьте статус валидатора и установите мониторинг позиции в очереди** с помощью опции `11`.

После обновления скрипта рекомендуется **удалить старый агент мониторинга** (опция `10`) и **создать новый** (опция `9`), чтобы получать актуальные уведомления (в т.ч. блок **L1 inclusion / Committee** в Telegram при наличии соответствующих данных). Для разбора calldata rollup агент использует Python-пакет `eth_abi`; скрипт проверяет зависимости и при необходимости устанавливает их через `pip3` или `python3 -m pip`.

Обязательно изучите другие возможности скрипта в меню, такие как:
*   Запуск и остановка контейнеров
*   Даунгрейд версии ноды (на случай проблем с обновлением)
*   Просмотр логов и статистики

## Добавление и удаление валидаторов без полной переустановки ноды (опции 24 и 25)

Если нода уже развёрнута и нужно изменить состав валидаторов **без** полной переустановки через опцию `13`:

*   **Опция 24** — добавить новых валидаторов (до 10 за один проход): обновляются `keystore.json`, файлы `validator_N.yml` в `$HOME/aztec/keys`, переменная `VALIDATORS` в `~/.env-aztec-agent`, при необходимости делается бэкап keystore, затем перезапускаются web3signer и `docker compose`. Формат и проверки BLS согласованы с уже существующим keystore; Ethereum-адреса приводятся к **нижнему регистру**; скрипт уточняет использование **publisher** (один на всех новых или свой на каждого) и feeRecipient/coinbase.
*   **Опция 25** — удалить валидаторов по номерам из показанного списка attester-адресов; нельзя удалить единственного валидатора; после удаления конфиги `validator_N.yml` перенумеровываются, список в `VALIDATORS` обновляется.

Следуйте подсказкам скрипта по бэкапу и перезапуску сервисов.

## Повторная установка ноды и добавление BLS ключей (опция 20)

При повторной установке ноды (опция 13) и при последующем добавлении BLS ключей важно понимать два варианта установки и то, как с ними работают подопции опции **20** (Генерация BLS ключей).

### Установка ноды: с BLS или без

- **С BLS ключами** — при установке (опция 13) на вопрос «У вас есть BLS ключи?» ответьте **`y`** и введите для каждого валидатора данные в формате:  
  `приватный_ключ,адрес,приватный_BLS`.  
  Скрипт сразу создаст `keystore.json` (BLS в JSON) и `validator_N.yml` только с SECP256K1. Отдельные `bls_validator_*.yml` не используются.

- **Без BLS ключей** — ответьте **`n`**. Скрипт создаст `keystore.json` только с eth-адресами валидаторов (без поля `bls`). BLS ключи можно добавить позже через опцию 20, как описано ниже.
...
### Вариант A: BLS из той же мнемоники, что и адреса в keystore (опции 20-2 и 20-3)
...
1. **Опция 20 → подопция 2** (существующие адреса из мнемоники).
...
3. Если нода была установлена **без BLS**, выполните **опция 20 → подопция 3** (добавить BLS ключи в keystore). Скрипт перенесёт BLS из `bls-filtered-pk.json` в `keystore.json`.
4. Дальше два пути:
    - **Вариант 1:** выполните **опции 21 (Approve)** и **22 (Stake)** для стейкинга валидатора.
    - **Вариант 2 (ключ для дашборда):** выполните **опция 20 → подопция 4** (dashboard keystores). Укажите **то же количество адресов**, что и в шаге 2 (например, 30). Скрипт создаст файлы нового формата со всеми сгенерированными адресами. Откройте созданный файл, **вручную удалите лишние записи** и оставьте только ту, которая соответствует нужному вам валидатору.

---

### Вариант B: Новый адрес оператора (опция 20-1)

Подходит, если вы хотите завести **новый** eth-адрес и BLS ключи (например, для перехода на новый кошелёк).

1. **Опция 20 → подопция 1** (новый адрес оператора).
...
3. Пополните новый eth-адрес Sepolia ETH (0.1–0.3), затем выполните **опции 21 (Approve)** и **22 (Stake)**. Опция 22 при успешном стейке **сама обновит `keystore.json`**, подставив новый адрес оператора вместо старого, и создаст YML-файлы с новыми ключами.
4. Опция **20-3** в этом сценарии не используется — подстановка адресов в keystore делается в опции 22.

---

### Вариант C: Только ключи для дашборда (опция 20-4)

Если нужны **только** keystore для staking dashboard (docs.aztec.network), без изменения конфигурации ноды:

- **Опция 20 → подопция 4.** Выберите новую мнемонику (1) или введите существующую (2), укажите количество идентичностей валидаторов. Скрипт создаст в `$HOME/aztec/` файлы **`dashboard_keystore.json`** и **`dashboard_keystore_staker_output.json`**. В `keystore.json` ноды и в `bls-filtered-pk.json` ничего не меняется.
...
### Краткая сводка по подопциям 20

| Подопция | Назначение | Меняет ли `keystore.json`? |
|----------|------------|----------------------------|
| **20-1** | Новый адрес оператора; потом 21 и 22 (22 сама обновит keystore) | Нет напрямую; обновление в опции 22 |
| **20-2** | BLS из мнемоники для адресов, уже указанных в keystore при установке | Нет; создаёт только `bls-filtered-pk.json` |
| **20-3** | Перенести BLS из `bls-filtered-pk.json` в `keystore.json` | Да |
| **20-4** | Keystore для дашборда (отдельные файлы в `$HOME/aztec/`) | Нет |

## Поиск неисправностей

### Нода Aztec не запускается с RPC от sepolia-auto-install и файрволлом (опция 13 скрипта sepolia-auto-install)

<details>
<summary>Решение</summary>

Если RPC установлен скриптом [sepolia-auto-install](https://github.com/pittpv/sepolia-auto-install/) и в нём активирован файрволл (опция 13 **этого** RPC-скрипта, не меню Aztec), нужно разрешить подсеть Docker-сети Aztec. Узнайте подсеть одной из команд:
