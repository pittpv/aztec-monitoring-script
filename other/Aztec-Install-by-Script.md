...
При повторной установке ноды (опция 11) и при последующем добавлении BLS ключей важно понимать два варианта установки и то, как с ними работают подопции опции **18** (Генерация BLS ключей из мнемоники).
...
Файл **`bls-filtered-pk.json`** создаётся и используется **только скриптом**. Его не нужно редактировать вручную, кроме особого случая с дашбордом (см. вариант 2 ниже). Путь: `$HOME/aztec/bls-filtered-pk.json`.
