...
## Добавление и удаление валидаторов без полной переустановки ноды (опции 25 и 26)

Если нода уже развёрнута и нужно изменить состав валидаторов **без** полной переустановки через опцию `11`:

*   **Опция 25** — добавить новых валидаторов (до 10 за один проход): обновляются `keystore.json`, файлы `validator_N.yml` в `$HOME/aztec/keys`, переменная `VALIDATORS` в `~/.env-aztec-agent`, при необходимости делается бэкап keystore, затем перезапускаются web3signer и `docker compose`. Формат и проверки BLS согласованы с уже существующим keystore; Ethereum-адреса приводятся к **нижнему регистру**; скрипт уточняет использование **publisher** (один на всех новых или свой на каждого) и feeRecipient/coinbase.
*   **Опция 26** — удалить валидаторов по номерам из показанного списка attester-адресов; нельзя удалить единственного валидатора; после удаления конфиги `validator_N.yml` перенумеровываются, список в `VALIDATORS` обновляется.
