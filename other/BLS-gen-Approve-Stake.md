# BLS: генерация ключей, Approve и Stake (опции 20, 21, 22)

Подробное описание работы опций **20** (генерация BLS-ключей и подопции), **21** (Approve) и **22** (Stake) в скрипте aztec-logs для стейкинга валидаторов Aztec.

---

## Опция 20 — Генерация BLS-ключей из мнемоники

В меню (категория «Стейкинг и валидаторы» или быстрый ввод номера) при выборе **20** открывается подменю **«BLS Keys Generation and Transfer»** с четырьмя способами работы с BLS-ключами.
...
### Подопция 20-1: Новый адрес оператора
...
- Дальнейшие шаги скрипт подсказывает: отправить 0.1–0.3 Sepolia ETH на новый адрес, затем выполнить **опцию 21 (Approve)** и **опцию 22 (Stake)**.

**Когда использовать:** Смена оператора валидатора (новый кошелёк и новые BLS-ключи при сохранении старых адресов в системе).

---

### Подопция 20-2: Существующие адреса (из одной мнемоники)
...
**Результат:** В `$HOME/aztec/bls-filtered-pk.json` — только ключи, соответствующие валидаторам из keystore. Формат «старый»: нет `new_operator_info`, только `attester.eth`, `attester.bls`, `feeRecipient`. Дальше можно выполнять **опцию 21** и **опцию 22** (старый формат стейкинга).

**Когда использовать:** Все валидаторы из одной сид-фразы, адреса уже есть в keystore — нужно только добавить/обновить BLS-ключи.

---

### Подопция 20-3: Добавить BLS-ключи в keystore.json
...
**Когда использовать:** Только после генерации BLS (подопция 20-1 или 20-2) и только если BLS получены из сид-фразы или вы сами корректно собрали `bls-filtered-pk.json`. Не применяйте к случайным файлам.

---

### Подопция 20-4: Keystore для дашборда (рекомендуется)
...
**Когда использовать:** Подготовка ключей именно для веб-дашборда стейкинга Aztec, а не для работы опций 21/22 в этом скрипте.

---

## Опция 21 — Approve

**Назначение:** Разрешить смарт-контракту стейкинга (rollup) тратить ваш стейкинг-токен (approve на сумму до 200000 ether в единицах токена). Без этого шага опция 22 (Stake) не сможет списать токен с вашего кошелька.
...
4. Для каждого ключа:
   ...
   - Определяет адрес стейкинг-токена через `getStakingAsset()` у rollup (при сбое — fallback mainnet `0xa27ec0006e59f245217ff08cd52a7e8b169e62d2`). Вызывает **ERC-20 approve**: метод `approve(address,uint256)` — spender = адрес rollup, сумма `200000 ether` (в wei токена).
...
**Порядок:** Approve (21) выполняется **после** генерации BLS (20) и **до** Stake (22). Для метода «новый оператор» (20-1) перед 21 нужно пополнить новые адреса ETH для газа.
...
1. Проверяет наличие `$HOME/aztec/bls-filtered-pk.json`. Если файла нет — предлагает сначала выполнить опцию 20.
2. Определяет формат bls-filtered-pk:
   - если у первого валидатора есть поле **`new_operator_info`** — считается **новый формат** (метод нового оператора, 20-1);
   - иначе — **старый формат** (существующие адреса, 20-2).
...
1. **Опция 20** — выбрать подходящий способ (1: новый оператор, 2: существующие адреса из мнемоники, 3: добавление BLS в keystore, 4: дашборд).
2. Для **20-1**: пополнить новые адреса на 0.1–0.3 Sepolia ETH.
3. **Опция 21** — Approve для всех ключей из `$HOME/aztec/keys/*.yml` (кроме bls).
4. **Опция 22** — Stake по данным из `bls-filtered-pk.json` (формат определяется автоматически).
