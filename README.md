...
- В меню BLS (опция 18) добавлена новая подопция: генерация keystore для дашборда — создаются файлы с префиксом `dashboard_` в `$HOME/aztec/` (приватный keystore и staker_output для staking dashboard), поддерживается новая мнемоника или ввод своей сид-фразы.
