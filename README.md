...
## 📌 Последние обновления 15-07-2026

⚠️ Обновите ноду до **Aztec v5** (образ `5.0.0+`), пересоберите docker-compose **без `--archiver`**. Удалите старый агент мониторинга (**опция 10**) и установите нового (**опция 9**).

- Совместимость с протоколом **Aztec v5 (AZUP-2)**
- Mainnet rollup: `0x91ff8bbd8ebb07893010d50a48a1609e5ebd8e34`
- Testnet rollup: `0xfe6061806cac748085904a010d2d9e33b8031741`
- Tip сети: `getPendingCheckpointNumber()`; событие **CheckpointProposed** (topic0 `0x6ff492bf…`)
- Декодер `propose()` **v5** (`0x72636df9`) для L1 committee в агенте
- Установка ноды: образ `5.0.0`, без `--archiver`, `GOVERNANCE_PROPOSER_PAYLOAD_ADDRESS`, `ETHEREUM_DEBUG_HOSTS`
- JSON-RPC: `aztec_getChainTips` / `aztec_*` с fallback на legacy `node_*`
- Approve через `getStakingAsset()`; Claim без `isRewardsClaimable` (нет в ABI v5), подпись gas-ключом
- В агенте: сначала `getEpochForCheckpoint`, fallback `getEpochForBlock`
- **Новый интерфейс меню**: категории вместо одного длинного списка; часть опций перенумерована (см. раздел «Использование»)
  - `↑` / `↓` — навигация, `Enter` — выбор
  - номер опции `0–27` + `Enter` — быстрый переход (из экрана категорий или подменю)
  - в подменю: пункт «← Назад к категориям»
  - строка статуса: версия скрипта, сеть, RPC
...
- Исправлена сигнатура TOPIC0 события CheckpointProposed для **мейннет**. Неверное значение приводило к тому, что проверка L1 не работала

### 30-03-2026

- Обновлен mainnet rollup-контракт `0xae2001f7e21d5ecabf6234e9fdd1e76f50f74962`
- Добавлена **Опция 27** для поиска и вывода ADMIN API KEY
- Добавлена зависимость **Node.js ≥24.12.0** и установка с согласия пользователя
- Обновлена **Опция 11** — установка Aztec ноды
  - Актуализирована логика работы с обновленным нативным скриптом установки бинарных файлов Aztec
  - Исправлена команда запуска web3signer (корректно устанавливается chain id для mainnet/testnet)

### 24-03-2026

- Блок **L1 inclusion / Committee** в Telegram: проверка включения транзакций валидатора в L1, заголовки и подписи слотов (нода и L1), легенда статусов по calldata rollup.
  - **Зависимости для проверки в L1**: проверка/установка Python-пакета `eth_abi`; установка pip-зависимостей через **`pip3_or_pym` / `pip3_or_pym_available`** (fallback на `python3 -m pip`).
  - aztec_l1_extract_signers.py - декордер calldata для L1 для скрипта агента мониторинга, создается автоматически при установке мониторинга
- Добавлены опции **25** и **26**: добавление и удаление валидаторов без полной переустановки ноды — keystore, YAML-ключи в `$HOME/aztec/keys`, `VALIDATORS` в `~/.env-aztec-agent`, бэкап keystore, перезапуск web3signer и docker compose.
  - Для опции **25**: ввод новых валидаторов с учётом BLS как у существующего keystore, лимит 10 валидаторов, проверка дубликатов, feeRecipient/coinbase (по умолчанию с первого валидатора), нормализация Ethereum-адресов в **lowercase**, вопрос про **текущий publisher** из keystore для всех новых или отдельный publisher на каждого.
  - Для опции **26**: нумерованный список attester-адресов, удаление по номерам через запятую, запрет снести единственного валидатора, перенумерация `validator_1.yml` … после удаления, вычитание адресов из `VALIDATORS`.
- **Переводы** новых и затронутых строк на **английский, русский и турецкий** (меню 25–26, L1/committee, `eth_abi`, правки формата валидаторов и пр.).
- Контракт rollup: вместо одной неиспользуемой сигнатуры — **`FUNCTION_SIG_BLOCK`** и **`FUNCTION_SIG_CHECKPOINT`** в cast-вызовах и в сгенерированном агенте (`check_blocks`).
- **Установка ноды**: публичные BLS-ключи убраны из логики установки. Приватные BLS-ключи используются напрямую в keystore.json (согласно официальной документации). Убрана генерация отдельных `bls_validator_*.yml`; BLS остаётся в keystore; формат **multi-validator** без поля public BLS, строка ввода public BLS убрана.

### 08-03-2026

- Обновлён адрес роллап-контракта в тестовой сети: `0xf6D0D42aCE06829bECB78C74F49879528fC632c1`.

⚠️ ТОЛЬКО ДЛЯ ТЕСТОВОЙ СЕТИ — После обновления скрипта используйте опцию даунгрейда для обновления ноды до версии `4.1.0-rc.2` в 20:30 UTC (7 марта) и в течение 24 ч после этого. Удалите старый агент мониторинга (опция 3) и создайте нового агента (опция 2).
...
3. **Следуйте инструкциям** для:
   ...
   - Активации мониторинга (**опция 9**)
   - Скачивания файла определений критических ошибок (**опция 27**)

❗️Для получения статистики по слотам в ноде **должен быть** установлен уровень логов: `info;debug:node:sentinel` или `debug`

## 🖥️ Использование

Меню разбито на категории. В интерактивном терминале:

- `↑` / `↓` — перемещение по пунктам, `Enter` — выбор
- наберите номер опции `0–27` и нажмите `Enter` для быстрого перехода (можно прямо с экрана категорий)
- в подменю выберите «← Назад к категориям», чтобы вернуться
- если терминал неинтерактивный, показывается плоский список 1–27 с вводом номера

### Диагностика и логи
1. Проверить контейнер и синхронизацию ноды  
2. Просмотреть логи Aztec  
3. Найти rollupAddress  
4. Найти PeerID  
5. Найти governanceProposerPayload  
6. Проверить Proven L2 блок  
7. Проверить версию ноды Aztec  
8. Найти Admin API key в логах  

### Мониторинг
9. Установить агент мониторинга ноды с уведомлениями  
10. Удалить агент мониторинга  
11. Поиск валидатора, проверка статуса и мониторинг очереди  
12. Мониторинг баланса publisher  

### Управление нодой
13. Установить Aztec ноду с Watchtower  
14. Удалить ноду Aztec  
15. Запустить контейнеры ноды Aztec  
16. Остановить контейнеры ноды Aztec  
17. Обновить ноду Aztec  
18. Сделать даунгрейд ноды Aztec  
19. Изменить RPC URL  

### Стейкинг и валидаторы
20. Сгенерировать BLS ключи из мнемоники  
21. Апрув  
22. Стейк  
23. Получить награды  
24. Добавить валидаторов  
25. Удалить валидаторов  

### Обновления и обслуживание
26. Проверить обновления скрипта (безопасно, с проверкой хеша)  
27. Проверить обновления определений ошибок (безопасно, с проверкой хеша)  

`0.` 🚪 Выход  

## 🚀 Использование агента мониторинга ноды

После установки скрипта выберите **опцию 9** (`Установить агент мониторинга ноды с уведомлениями`):
...
❗️При **первом** запуске необходимо использовать **опцию 27**, чтобы скачать файл с критическими ошибками `error_definitions.json`
...
Для установки ноды Aztec выберите **опцию 13** и следуйте инструкциям скрипта.
